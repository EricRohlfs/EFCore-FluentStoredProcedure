...
namespace Tests.Extensions.ProcedureBuilderTests
{
    ...
    public class MockDatabaseFactory : DbProviderFactory
    {
        ...
        public static Mock<MoqDataReader> CreateDbDataReader(TestDataContainer dataContainer)
        {
            ...
            DbCommand = CreateDbCommand(dataReader.Object);
            return dataReader;
        }
        ...
        public static void SetScalerReturnValue(object value)
        {
            if (DbCommand != null)
            {
                DbCommand.Setup(dbc => dbc.ExecuteScalar()).Returns(value);
                DbCommand.Setup(dbc => dbc.ExecuteScalarAsync(It.IsAny<System.Threading.CancellationToken>()))
                   .ReturnsAsync(value);
            }
        }
        public static Mock<DbCommand> CreateDbCommand(DbDataReader dataReader = null)
        {
            ...
            //dbCommand.Protected().Setup<DbParameter>("CreateDbParameter")
            //   .Returns(
            //      DbParameter.Object);

            dbCommand.CallBase = true;
            dbCommand.SetupProperty(dbc => dbc.CommandText);
            dbCommand.SetupProperty(dbc => dbc.CommandType);
            return dbCommand;
        }
    }
